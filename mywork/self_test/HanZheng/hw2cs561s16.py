...
def FOL_BC_OR(kb, goal, substs):
	...
	global Query_Waiting_Ans
	old_sub, flag = substs, 0
	args = [arg if isinstance(arg, Constant) else '_' for arg in goal.args]
	# prevent infinite loop
	ask_print = '%s(%s)\n' % (goal.func, ', '.join(args))
	if ask_print in Query_Waiting_Ans:
		output_file.write('False: %s' % ask_print)		
		return
	for (lhs,rhs,code) in FATCH_RULE_FOR_GOAL(kb, goal):
		#undo last sub
		substs = old_sub
		Query_Waiting_Ans.add(ask_print)
		output_file.write('Ask: %s' % ask_print)
		if code == -1:
			break
		if code == 0:
			...
			for sub_and in FOL_BC_AND(kb, lhs, UNIFY(rhs[0], goal, substs)):
				...
				Query_Waiting_Ans.discard(ask_print)
